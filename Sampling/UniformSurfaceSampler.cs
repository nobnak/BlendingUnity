...
namespace nobnak.Sampling {
    public class UniformSurfaceSampler {
        public Mesh M { get; private set; }

        private float[] _cumWeights;

		public UniformSurfaceSampler(Mesh m, float[] cumWeights) {
            this.M = m;
		...
		public static UniformSurfaceSampler Generate(GameObject g) {
			var mf = g.GetComponent<MeshFilter>();
			if (mf == null || mf.sharedMesh == null)
				return null;
			return Generate(mf.sharedMesh);
		}
		public static UniformSurfaceSampler Generate(Mesh m) {
			...
			return new UniformSurfaceSampler(m, cumWeights);
        }
